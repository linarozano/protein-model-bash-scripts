#!bin/bash
#Lrlrlr restraint.sh 051220 edited 230221 edited 240321 custom forloop edited 090921 cutoff 5A.
#inclusive with remove header function. input files expected to have headers.

for x in ./*.txt
do
	sed -i -e "1d" $x	#remove header (optional)
	cat $x | sed 's!/! !g'| sed 's!`! !g' > main #split everything into columns
	...
	tail -n +1  ResNumber_chainI ClusPro_chainI ResNumber_chainII ClusPro_chainII > $x.residues5
